...
### V. 비즈니스 프로세스 오케스트레이션의 법칙 (BPO)

오케스트레이터는 프로세스의 마스터입니다. 작업의 전체 컨텍스트를 아는 유일한 구성 요소입니다. 작업의 전체 컨텍스트를 기반으로 결정을 내릴 수 있는 유일한 구성 요소입니다.
...
프롬프트는 코드입니다. 버전 제어 하에 있어야 합니다. 내부 지침에 대한 변경은 아무리 사소하더라도 에이전트의 새로운 불변 버전(예: 사소한 텍스트 조정의 경우 v1.0.0, 모델 전환 또는 로직 변경의 경우 v2.0.0)을 구성합니다. 프로덕션에서 "핫" 변경은 없습니다.
...
```mermaid
graph TD
    subgraph Orchestrator [비즈니스 프로세스 오케스트레이터]
        Start((RFP 시작)) --> Step1[에이전트 A 호출: 추출기]
        Step1 --> Validation1{계약 확인}
        Validation1 -->|성공| Legacy[ERP 조회: 가격 & 재고]
        Validation1 -->|실패| Error1[중단: 계약 위반]
        Legacy --> Step2[에이전트 B 호출: 위험 감사자]
        Step2 --> Validation2{회로 차단기?}
        Validation2 -->|위험 높음| Error2[중단: 위험 감지됨]
        Validation2 -->|위험 낮음| Step3[에이전트 C 호출: 최종 작성자]
        Step3 --> Final((최종 제안))
    end
    subgraph Agents [원자적 마이크로에이전트]
        AgentA[[에이전트 A: 추출기]]
        AgentB[[에이전트 B: 감사자]]
        AgentC[[에이전트 C: 작성자]]
    end
...
리포지토리에 MAS-Ready 배지를 추가하여 이 표준을 채택하십시오.
