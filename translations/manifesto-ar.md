...
### V. قانون تنسيق عمليات الأعمال (BPO)

المنسق هو سيد العملية. إنه المكون الوحيد الذي يعرف السياق الكامل للعملية. إنه المكون الوحيد الذي يمكنه اتخاذ القرارات بناءً على السياق الكامل للعملية.
...
المطالبة (Prompt) هي كود برمجي. يجب أن يكون خاضعًا للتحكم في الإصدار. أي تغيير في تعليمات داخلية، مهما كان صغيراً، يشكل إصداراً جديداً غير قابل للتغيير للوكيل (مثلاً، v1.0.0 للتعديلات النصية الطفيفة، v2.0.0 لتبديل النموذج أو تغييرات المنطق). لا توجد تغييرات "ساخنة" في الإنتاج.
...
```mermaid
graph TD
    subgraph Orchestrator [منسق عمليات الأعمال]
        Start((RFP بدء)) --> Step1[استدعاء الوكيل A: المستخرج]
        Step1 --> Validation1{فحص العقد}
        Validation1 -->|نجاح| Legacy[استعلام ERP: الأسعار والمخزون]
        Validation1 -->|فشل| Error1[توقف: خرق العقد]
        Legacy --> Step2[استدعاء الوكيل B: مدقق المخاطر]
        Step2 --> Validation2{قاطع الدائرة؟}
        Validation2 -->|مخاطر عالية| Error2[توقف: تم اكتشاف مخاطر]
        Validation2 -->|مخاطر منخفضة| Step3[استدعاء الوكيل C: الصياغة النهائية]
        Step3 --> Final((الاقتراح النهائي))
    end
    subgraph Agents [وكلاء ميكرو ذريين]
        AgentA[[الوكيل A: المستخرج]]
        AgentB[[الوكيل B: المدقق]]
        AgentC[[الوكيل C: الصائغ]]
    end
    Step1 -.-> AgentA
    Step2 -.-> AgentB
    Step3 -.-> AgentC
...
اعتمد هذا المعيار عن طريق إضافة شارة MAS-Ready إلى المستودع الخاص بك.
